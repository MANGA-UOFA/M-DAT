...
@register_task("translation_multilingual_nat_bt")
class TranslationMultilingualNatBT(LegacyFairseqTask):
    """
    ...
    def build_generator(self, models, args, **unused):
        ...
        from fs_plugins.others.multilingual_iterative_refinement_generator_bt import MultilingualIterativeRefinementGeneratorWithBacktranslation
        _, tgt_langtok_spec = self.args.langtoks["main"]
        return MultilingualIterativeRefinementGeneratorWithBacktranslation(
    ...
    def build_model(self, args, from_checkpoint=False):
        ...
        model.langtok_dict = self.data_manager.get_all_decoder_langtok_as_dict(tgt_langtok_spec)
        return model
    ...
    def train_step(
        ...
        sample['doing_bt'] = getattr(self.args, 'bt', False)
        sample['bt_threshold'] = getattr(self.args, 'bt_start_from', 10000)
    ...
    def filter_indices_by_size(
        ...
        if is_training:
            ...
            indices, ignored = dataset.filter_indices_by_size(indices, max_positions, 
                        src_upsample_scale=src_upsample_scale, min_sent_length=min_sent_length, training=is_training)
        if len(ignored) > 0:
