...
@register_criterion("multilingual_nat_dag_zeroshot_real_bt_loss_fix")
class NATDAGLoss(FairseqCriterion):
    ...
    def add_args(parser):
        ...
        parser.add_argument("--pivot-bt", action="store_true")
        parser.add_argument("--bt-glat", action="store_true")
        parser.add_argument("--bt-model-eval", action="store_true")

        parser.add_argument("--random-bt", action="store_true")
    ...
    def _encoder_consistency_loss(self, encoder_out1_loggits, encoder_out2_loggits):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        outputs = model(src_tokens, src_lengths, prev_output_tokens, tgt_tokens, glat, glat_function, src_lang_id=src_lang_id, tgt_lang_id=tgt_lang_id, doing_bt=False, mtl_ltc=False)
        ...
        if self.cfg.encoder_consistency_loss:
            ...
            if sum(new_src_lengths_cond) > 0:
                ...
                new_tgt_lang_id = src_lang_id.clone()[new_src_lengths_cond]
                new_src_tokens[:, 0] = new_tgt_lang_id
                new_encoder_out = model.forward_encoder(new_src_tokens, new_src_lengths)
                encoder_out1_loggits = outputs['encoder_out']['encoder_out'][0].permute(1, 0, 2)[new_src_lengths_cond][:, 1:, :].mean(1)
                encoder_out2_loggits = new_encoder_out['encoder_out'][0].permute(1, 0, 2)[:, 1:, :].mean(1)

                _enc_consist_loss = self._encoder_consistency_loss(encoder_out1_loggits, encoder_out2_loggits)
            else:
        ...
        pivot_bt = getattr(self.cfg, 'pivot_bt', False)

        # if pivot_bt:
        #     rand_seed = outputs['rand_seed']
        #     en_id = model.langtok_dict['en']
        #     # two groups:
        #     #  1. for en -> xx just do a random seq-level bt
        #     #  2. for xx -> en, do xx-en then en-xx

        #     # 1.1 Get random tgt_ids
        #     num_rand_langs = model.num_language - 1
        #     seq_langtokens = torch.randint(num_rand_langs, (tgt_lang_id.size()), device=tgt_lang_id.device, dtype=tgt_lang_id.dtype) + model.langtok_offset 
        #     offset_cond = (seq_langtokens>=src_lang_id)
        #     seq_langtokens[offset_cond] = seq_langtokens[offset_cond] + 1
        #     random_tgt_ids = seq_langtokens
            
        #     is_src_en_idx = (src_lang_id == en_id)

        #     new_tokens = model.dag_decode_get_tokens(self, src_tokens, src_lengths, rand_seed=rand_seed, tgt_lang_id=random_tgt_ids)
        ...
        #         ## get XX->EN 
        #         new_src_tokens = src_tokens.clone()[is_tgt_xx_idx]
        #         new_src_lengths = src_lengths.clone()[is_tgt_xx_idx]
        #         ori_tgt_lang_id = tgt_lang_id.clone()[is_tgt_xx_idx]
        #         new_tgt_tokens = tgt_tokens.clone()[is_tgt_xx_idx]
        ...
        # BT LOSS 
        bt_threshold = sample.get("bt_threshold", 10000)
        pivot_bt = getattr(self.cfg, 'pivot_bt', False)
        bt_model_eval = getattr(self.cfg, 'bt_model_eval', False)
        random_bt = getattr(self.cfg, 'random_bt', False)
        _bt_losses = None
        bt_dag_nll_loss = None
        zero_valid_bt_sample = False
        if is_training and doing_bt and update_num > bt_threshold: # 
            pad_index = model.pad
            with torch.no_grad():
                if bt_model_eval:
                    model.eval()
                # Get [ src_lang_id ; Y ] -> X hat
                if pivot_bt:
                    # two groups:
                    #  1. for en -> xx just do a random seq-level bt
                    #  2. for xx -> en, do xx-en then en-xx

                    # 0 Get random tgt_ids
                    infer_src_lang_id = sample['net_input']['tgt_lang_id'].clone()
                    infer_src_tokens= sample['target'].clone()

                    en_id = model.langtok_dict['en']
                    num_rand_langs = model.num_language - 1
                    seq_langtokens = torch.randint(num_rand_langs, (infer_src_lang_id.size()), device=infer_src_lang_id.device, dtype=infer_src_lang_id.dtype) + model.langtok_offset 
                    offset_cond = (seq_langtokens>=infer_src_lang_id)
                    seq_langtokens[offset_cond] = seq_langtokens[offset_cond] + 1
                    random_infer_tgt_lang_id = seq_langtokens
                    
                    # X_hat = model.dag_decode_get_tokens(infer_src_tokens, infer_src_tokens_lengths, tgt_lang_id=infer_tgt_lang_id)

                    # 1. SRC is EN group

                    is_src_en_idx_cond = torch.logical_or((infer_src_lang_id == en_id), (random_infer_tgt_lang_id == en_id))

                    if sum(is_src_en_idx_cond) > 0:
                        src_en_infer_src_tokens = infer_src_tokens[is_src_en_idx_cond]
                        src_en_infer_tgt_lang_id = random_infer_tgt_lang_id[is_src_en_idx_cond]
                        src_en_infer_src_tokens[:, 0] = src_en_infer_tgt_lang_id
                        src_en_infer_src_lengths = src_en_infer_src_tokens.ne(pad_index).sum(-1)


                        src_en_infer_src_lengths_cond = (src_en_infer_src_lengths < 256)
                        src_en_infer_src_tokens = src_en_infer_src_tokens[src_en_infer_src_lengths_cond]
                        src_en_infer_src_lengths = src_en_infer_src_lengths[src_en_infer_src_lengths_cond]
                        src_en_infer_tgt_lang_id = src_en_infer_tgt_lang_id[src_en_infer_src_lengths_cond]
                        
                        if len(src_en_infer_tgt_lang_id) > 0:
                            src_en_X_hat = model.dag_decode_get_tokens(src_en_infer_src_tokens, src_en_infer_src_lengths, tgt_lang_id=src_en_infer_tgt_lang_id)
                            
                            src_en_bt_src_tokens = src_en_X_hat
                            src_en_bt_tgt_lang_id = sample['net_input']['tgt_lang_id'][is_src_en_idx_cond][src_en_infer_src_lengths_cond]

                            src_en_bt_src_tokens = torch.cat( (src_en_bt_tgt_lang_id.unsqueeze(1), src_en_bt_src_tokens), dim=1)
                            # src_en_bt_src_lengths = src_en_bt_src_tokens.ne(pad_index).sum(-1)
                            src_en_bt_tgt_tokens = sample['target'][is_src_en_idx_cond][src_en_infer_src_lengths_cond]
                        else:
                            src_en_bt_src_tokens = None
                            src_en_bt_tgt_lang_id = None
                            src_en_bt_tgt_tokens = None
                    else:
                        src_en_bt_src_tokens = None
                        src_en_bt_tgt_lang_id = None
                        src_en_bt_tgt_tokens = None
                    ...
                    # 2. SRC is XX group
                    # 2.1 xx -> en
                    is_src_xx_idx_cond = torch.logical_not(is_src_en_idx_cond)
                    if sum(is_src_xx_idx_cond) > 0:
                        src_xx_to_en_infer_src_tokens = infer_src_tokens[is_src_xx_idx_cond]
                        src_xx_to_en_infer_tgt_lang_id = torch.ones_like(infer_src_lang_id) * en_id  # to english first
                        src_xx_to_en_infer_tgt_lang_id = src_xx_to_en_infer_tgt_lang_id[is_src_xx_idx_cond]
                        src_xx_to_en_infer_src_tokens[:, 0] = src_xx_to_en_infer_tgt_lang_id
                        src_xx_to_en_infer_src_lengths = src_xx_to_en_infer_src_tokens.ne(pad_index).sum(-1)

                        src_xx_to_en_infer_src_lengths_cond = (src_xx_to_en_infer_src_lengths < 256)
                        src_xx_to_en_infer_src_tokens = src_xx_to_en_infer_src_tokens[src_xx_to_en_infer_src_lengths_cond]
                        src_xx_to_en_infer_src_lengths = src_xx_to_en_infer_src_lengths[src_xx_to_en_infer_src_lengths_cond]
                        src_xx_to_en_infer_tgt_lang_id = src_xx_to_en_infer_tgt_lang_id[src_xx_to_en_infer_src_lengths_cond]
                        
                        if len(src_xx_to_en_infer_src_tokens) > 0:
                            src_xx_to_en_X_hat = model.dag_decode_get_tokens(src_xx_to_en_infer_src_tokens, src_xx_to_en_infer_src_lengths, tgt_lang_id=src_xx_to_en_infer_tgt_lang_id)      
                            # 2.2 en -> xx2

                            src_en_to_xx_infer_src_tokens = src_xx_to_en_X_hat
                            src_en_to_xx_tgt_lang_id = random_infer_tgt_lang_id[is_src_xx_idx_cond][src_xx_to_en_infer_src_lengths_cond]
                            src_en_to_xx_infer_src_tokens = torch.cat( (src_en_to_xx_tgt_lang_id.unsqueeze(1), src_en_to_xx_infer_src_tokens), dim=1)
                            src_en_to_xx_infer_src_lengths = src_en_to_xx_infer_src_tokens.ne(pad_index).sum(-1)

                            src_en_to_xx_infer_src_lengths_cond = (src_en_to_xx_infer_src_lengths < 256)
                            src_en_to_xx_infer_src_tokens = src_en_to_xx_infer_src_tokens[src_en_to_xx_infer_src_lengths_cond]
                            src_en_to_xx_infer_src_lengths = src_en_to_xx_infer_src_lengths[src_en_to_xx_infer_src_lengths_cond]
                            src_en_to_xx_tgt_lang_id = src_en_to_xx_tgt_lang_id[src_en_to_xx_infer_src_lengths_cond]

                            if len(src_en_to_xx_infer_src_tokens) > 0:
                                src_en_to_xx_X_hat = model.dag_decode_get_tokens(src_en_to_xx_infer_src_tokens, src_en_to_xx_infer_src_lengths, tgt_lang_id=src_en_to_xx_tgt_lang_id)

                                
                                src_xx_bt_src_tokens = src_en_to_xx_X_hat
                                src_xx_bt_tgt_lang_id = sample['net_input']['tgt_lang_id'][is_src_xx_idx_cond][src_xx_to_en_infer_src_lengths_cond][src_en_to_xx_infer_src_lengths_cond]

                                src_xx_bt_src_tokens = torch.cat((src_xx_bt_tgt_lang_id.unsqueeze(1), src_xx_bt_src_tokens), dim=1)
                                src_xx_bt_tgt_tokens = sample['target'][is_src_xx_idx_cond][src_xx_to_en_infer_src_lengths_cond][src_en_to_xx_infer_src_lengths_cond]
                            else:
                                src_xx_bt_src_tokens = None
                                src_xx_bt_tgt_lang_id = None
                                src_xx_bt_tgt_tokens = None
                        else:
                            src_xx_bt_src_tokens = None
                            src_xx_bt_tgt_lang_id = None
                            src_xx_bt_tgt_tokens = None
                        ...
                        src_xx_bt_tgt_lang_id = None
                        src_xx_bt_tgt_tokens = None
                    # print(src_en_bt_src_tokens is None, src_xx_bt_src_tokens is None, flush=True)
                    if src_en_bt_src_tokens is not None and src_xx_bt_src_tokens is None:
                        bt_src_tokens = src_en_bt_src_tokens
                        bt_src_lengths = bt_src_tokens.ne(pad_index).sum(-1)
                        bt_tgt_lang_id = src_en_bt_tgt_lang_id
                        bt_tgt_tokens = src_en_bt_tgt_tokens
                        bt_tgt_lengths = bt_tgt_tokens.ne(pad_index).sum(-1)

                    elif src_en_bt_src_tokens is None and src_xx_bt_src_tokens is not None:
                        bt_src_tokens = src_xx_bt_src_tokens
                        bt_src_lengths = bt_src_tokens.ne(pad_index).sum(-1)
                        bt_tgt_lang_id = src_xx_bt_tgt_lang_id
                        bt_tgt_tokens = src_xx_bt_tgt_tokens
                        bt_tgt_lengths = bt_tgt_tokens.ne(pad_index).sum(-1)

                    elif src_en_bt_src_tokens is None and src_xx_bt_src_tokens is None:
                        zero_valid_bt_sample = True

                    else:
                        src_max_len = max(src_en_bt_src_tokens.size(1), src_xx_bt_src_tokens.size(1))
                        if src_max_len > src_en_bt_src_tokens.size(1):
                            src_en_bt_src_tokens = torch.cat((src_en_bt_src_tokens, torch.ones((src_en_bt_src_tokens.size(0), src_max_len-src_en_bt_src_tokens.size(1))).to(src_en_bt_src_tokens)*pad_index ), dim=-1)
                        if src_max_len > src_xx_bt_src_tokens.size(1):
                            src_xx_bt_src_tokens = torch.cat((src_xx_bt_src_tokens, torch.ones((src_xx_bt_src_tokens.size(0), src_max_len-src_xx_bt_src_tokens.size(1))).to(src_xx_bt_src_tokens)*pad_index ), dim=-1)

                        bt_src_tokens = torch.cat((src_en_bt_src_tokens, src_xx_bt_src_tokens), dim=0)
                        bt_src_lengths = bt_src_tokens.ne(pad_index).sum(-1)
                        bt_tgt_lang_id = torch.cat((src_en_bt_tgt_lang_id, src_xx_bt_tgt_lang_id), dim=0)
                        bt_tgt_tokens = torch.cat((src_en_bt_tgt_tokens, src_xx_bt_tgt_tokens), dim=0)
                        bt_tgt_lengths = bt_tgt_tokens.ne(pad_index).sum(-1)
                
                elif random_bt:
                    infer_src_lang_id = sample['net_input']['tgt_lang_id'].clone()
                    infer_src_tokens= sample['target'].clone()
                    infer_src_tokens_lengths = infer_src_tokens.ne(pad_index).sum(-1)
                    ...
                    seq_langtokens[offset_cond] = seq_langtokens[offset_cond] + 1
                    random_infer_tgt_lang_id = seq_langtokens
                    if len(infer_src_tokens) > 0:
                        X_hat = model.dag_decode_get_tokens(infer_src_tokens, infer_src_tokens_lengths, tgt_lang_id=random_infer_tgt_lang_id)
                        
                        bt_src_tokens = X_hat
                        bt_tgt_lang_id = sample['net_input']['tgt_lang_id'].clone()
                        bt_src_tokens = torch.cat( (bt_tgt_lang_id.unsqueeze(1), bt_src_tokens), dim=1)
                        bt_src_lengths = bt_src_tokens.ne(pad_index).sum(-1)

                        bt_tgt_tokens = sample['target'].clone()
                        bt_tgt_lengths = bt_tgt_tokens.ne(pad_index).sum(-1)
                    else:
                        zero_valid_bt_sample = True

                else:
                    infer_tgt_lang_id = sample['net_input']['src_lang_id'].clone()
                    infer_src_tokens= sample['target'].clone()
                    infer_src_tokens[:, 0] = infer_tgt_lang_id 
                    infer_src_tokens_lengths = infer_src_tokens.ne(pad_index).sum(-1)
                    X_hat = model.dag_decode_get_tokens(infer_src_tokens, infer_src_tokens_lengths, tgt_lang_id=infer_tgt_lang_id)
                ...
                if bt_model_eval:
                    model.train()
            # zero_valid_bt_sample = True
            # print('zero_valid_bt_sample', zero_valid_bt_sample, flush=True)
            LEN_FILTER_RATIO = 2
            if not zero_valid_bt_sample:
                filter_cond = torch.logical_and(bt_src_lengths * LEN_FILTER_RATIO > bt_tgt_lengths, 
                                                bt_tgt_lengths * LEN_FILTER_RATIO > bt_src_lengths)
                filter_cond = torch.logical_and(filter_cond, bt_src_lengths < 256)
                filter_cond = torch.logical_and(filter_cond, bt_tgt_lengths < 256)
                if filter_cond.sum() > 0 :
    
                    bt_tgt_lang_id = bt_tgt_lang_id[filter_cond]
                    bt_src_tokens = bt_src_tokens[filter_cond]
                    bt_src_lengths = bt_src_lengths[filter_cond]
                    
                    bt_tgt_tokens = bt_tgt_tokens[filter_cond]

                    
                    bt_prev_output_tokens = model.initialize_output_tokens_by_tokens(bt_src_tokens, bt_tgt_tokens, None)

                    bt_glat = glat if  getattr(model.args, 'bt_glat', False) else None
                    bt_outputs = model(bt_src_tokens, bt_src_lengths, bt_prev_output_tokens, bt_tgt_tokens, bt_glat, glat_function, src_lang_id=None, tgt_lang_id=bt_tgt_lang_id)
                    ...
                    # print('I should not be am HERE!!!', flush=True)
